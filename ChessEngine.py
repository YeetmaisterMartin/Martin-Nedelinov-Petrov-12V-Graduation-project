class GameState:
    ...
    def make_move(self, move):
        ...
        if move.is_pawn_promotion:
            # if not is_AI:
            #    promoted_piece = input("Promote to Q, R, B, or N:") # still in progress
            #    self.board[move.end_row][move.end_col] = move.piece_moved[0] + promoted_piece
            # else:
            self.board[move.end_row][move.end_col] = move.piece_moved[0] + "Q" # right now promotion is just a queen
            ...
            if len(self.checks) == 1:  # only 1 check, block the check or move the king
                ...
                if piece_checking[1] == "N":
                    ...
                    for i in range(1, 8):
                        ...
                        if valid_square[0] == check_row and valid_square[1] == check_col:  # get to piece and check
                            break
                # get rid of any moves that don't block check or move king
                for i in range(len(moves) - 1, -1, -1):  # iterate through the list backwards when removing elements
                    if moves[i].piece_moved[1] != "K":  # move doesn't move king, it must block or capture
    ...
    def get_knight_moves(self, row, col, moves):
        ...
        knight_moves = ((-2, -1), (-2, 1), (-1, 2), (1, 2), (2, -1), (2, 1), (-1, -2),
                        (1, -2))  # up/left up/right right/up right/down/left down/right left/up left/down
    ...
    def get_chess_notation(self):
        ...
        if self.is_castle_move:
            if self.end_col == 1:
                ...
                return "0-0"
        if self.is_enpassant_move:
            return self.get_rank_file(self.start_row, self.start_col)[0] + \
                "x" + self.get_rank_file(self.end_row, self.end_col) + " e.p."
